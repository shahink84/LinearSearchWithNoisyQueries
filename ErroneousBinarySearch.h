...
public:
    ...
    The output is the rihgtmost descendant of the node of the tree that is guaranteed to contain x in its subtree */
    int bs_limited_query_rightmost_child(int x, bool* answers, int H, bool printLog)
    {
        int root = T->getRoot();
        int cur_node = root;
        ...
        // Return the rightmost leaf
        cur_node = T->getRightmostLeaf(cur_node);
